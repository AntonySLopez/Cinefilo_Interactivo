...
async function getpreferencias(request) {
    try{
...
async function updatepreferencias(request) {
    try {
        const data = await request.json();
        ...
        return new Response(JSON.stringify({
            code: 500,
            message: 'Error update-preferencias en base de datos'
        }), { status: 500 });
    }
}
...
export { getpreferencias , updatepreferencias };
